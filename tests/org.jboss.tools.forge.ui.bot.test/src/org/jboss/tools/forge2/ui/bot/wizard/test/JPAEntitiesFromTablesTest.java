...
public class JPAEntitiesFromTablesTest extends WizardTestBase {
	...
	public void prepare() {
		...
		new LabeledText("Connection Name:").setText(PROFILE_NAME);
		new LabeledText("JDBC URL:").setText(SAKILA_URL);
		new LabeledText("User Name:").setText(SAKILA_USERNAME);
		new DefaultCombo(0).setSelection("H2 Database : org.hibernate.dialect.H2Dialect");
		new LabeledText("Driver Location:").setText(dbFolder + "/" + SAKILA_H2_DRIVER);
	...
	public void testGenerateEntities() {
		...
		new LabeledText("Target package:").setText(PACKAGE);
		DefaultCombo profileCombo = new DefaultCombo(0);
		assertTrue("Missing connection profile selection", profileCombo.getItems().contains(PROFILE_NAME));
		profileCombo.setSelection(PROFILE_NAME);
		dialog.next();

		Table table = new DefaultTable();
		assertFalse("No database tables found", table.getItems().isEmpty());
